...
def change_bh_mass_bondi(disk_bh_pro_masses, disk_bh_pro_orb_a, disk_density, disk_sound_speed, timestep_duration_yr):
    ...
    """
    Adds mass according to the Bondi accretion rate

    Takes initial BH masses at the start of the timestep and adds mass
    according to the Bondi accretion rate.
    ...
    disk_sound_speed : function
        Sound speed of the medium [cm/s]
    timestep_duration_yr : float
        Length of timestep [yr]
    ...
    -------
    disk_bh_pro_new_masses : numpy.ndarray
        Masses [M_sun] of black holes after accreting at Bondi rate for one timestep with :obj:`float` type
    """
    # Constants
    G = 6.67430e-8  # Gravitational constant [cm^3/g/s^2]
    yr_to_s = 3.154e7  # Seconds in a year

    rho = disk_density(disk_bh_pro_orb_a) * 1.e3
    Cs = disk_sound_speed(disk_bh_pro_orb_a)
    ...
    # Compute new masses after accretion for one timestep
    disk_bh_pro_new_masses = disk_bh_pro_masses + bondi_rate_yr * timestep_duration_yr

    return disk_bh_pro_new_masses

def change_bh_spin_magnitudes_bondi(disk_bh_pro_spins,
                                    disk_bh_masses,
                                    disk_bh_orb_a,
                                    disk_density,
                                    disk_sound_speed,
                                    ...
                                    disk_bh_pro_orbs_ecc_crit):
    """
    Updates the spin magnitude of the embedded black holes based on their accreted mass in this timestep using the Bondi accretion rate.

    Parameters
    ----------
    disk_bh_pro_spins : numpy.ndarray
        Initial spins [unitless] of black holes in prograde orbits around SMBH
    disk_bh_masses : numpy.ndarray
        Masses [M_sun] of black holes in prograde orbits around SMBH
    disk_density : function
        Density of the medium [g/cm^3].
    disk_sound_speed : function
        Sound speed of the medium [cm/s].
    ...
    rho = disk_density(disk_bh_orb_a) * 1.e3
    Cs = disk_sound_speed(disk_bh_orb_a)

    # Bondi accretion rate [M_sun/s]
    bondi_rate = (4 * np.pi * G ** 2 * disk_bh_masses ** 2 * rho) / Cs ** 3
    bondi_rate_yr = bondi_rate * yr_to_s  # Convert to [M_sun/yr]

    # Accreted mass during the timestep
    accreted_mass = bondi_rate_yr * timestep_duration_yr
    ...
    # Magnitude of spin iteration per normalized timestep
    #spin_iteration = (1.e-3 * accreted_mass / disk_bh_masses * normalized_spin_torque_condition * normalized_timestep)

    disk_bh_pro_spins_new = disk_bh_pro_spins.copy()
    ...
    # disk_bh_pro_spins_new[prograde_orb_ang_mom_indices]=disk_bh_pro_spins_new[prograde_orb_ang_mom_indices]+(4.4e-3*normalized_Eddington_ratio*normalized_spin_torque_condition*normalized_timestep)
    #disk_bh_pro_spins_new[indices_bh_spin_up] = disk_bh_pro_spins[indices_bh_spin_up] + (1.e-3 * (accreted_mass[indices_bh_spin_up] / disk_bh_masses[indices_bh_spin_up]) * normalized_spin_torque_condition * normalized_timestep)

    for i in indices_bh_spin_up:
        spin_iteration = (1.e-3 * (accreted_mass[i] / disk_bh_masses[i]) * normalized_spin_torque_condition * normalized_timestep)
        disk_bh_pro_spins_new[i] += spin_iteration

    for i in indices_bh_spin_down:
        spin_iteration = (1.e-3 * (accreted_mass[i] / disk_bh_masses[i]) * normalized_spin_torque_condition * normalized_timestep)
        disk_bh_pro_spins_new[i] -= spin_iteration

    # Spin down BH with orb ecc > disk_bh_pro_orbs_ecc_crit
    #disk_bh_pro_spins_new[indices_bh_spin_down] = disk_bh_pro_spins[indices_bh_spin_down] - (1.e-3 * (accreted_mass[indices_bh_spin_down] / disk_bh_masses[indices_bh_spin_down]) * normalized_spin_torque_condition * normalized_timestep)
...
def change_bh_spin_angles_bondi(disk_bh_pro_spin_angles,
                                disk_bh_masses,
                                disk_bh_orb_a,
                                disk_density,
                                disk_sound_speed,
                                ...
                                disk_bh_pro_orbs_ecc_crit):
    """
    Updates the spin angles of the embedded black holes based on their accreted mass in this timestep using the Bondi accretion rate.
    ...
    # Calculate change in spin angle due to accretion during timestep
    # spin_torque_iteration = (6.98e-3 * accreted_mass / disk_bh_masses * normalized_spin_torque_condition * normalized_timestep)
    ...
    for i in indices_bh_spin_up:
        spin_iteration = (6.98e-3 * (accreted_mass[i] / disk_bh_masses[i]) * normalized_spin_torque_condition * normalized_timestep)
        disk_bh_spin_angles_new[i] -= spin_iteration
    ...
    # Spin up BH are torqued towards zero (ie alignment with disk, so decrease mag of spin angle)
    #disk_bh_spin_angles_new[indices_bh_spin_up] = disk_bh_pro_spin_angles[indices_bh_spin_up] - (6.98e-3 * (accreted_mass[indices_bh_spin_up] / disk_bh_masses[indices_bh_spin_up]) * normalized_spin_torque_condition * normalized_timestep)
    # Spin down BH with orb ecc > disk_bh_pro_orbs_ecc_crit are torqued toward anti-alignment with disk, incr mag of spin angle.
    #disk_bh_spin_angles_new[indices_bh_spin_down] = disk_bh_pro_spin_angles[indices_bh_spin_down] + (6.98e-3 * (accreted_mass[indices_bh_spin_up] / disk_bh_masses[indices_bh_spin_up]) * normalized_spin_torque_condition * normalized_timestep)
    # print(disk_bh_spin_angles_new[indices_bh_spin_down])
