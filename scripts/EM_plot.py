...
def temp_fxn(v_fs, dens, m_p, c, k):
    """
    Calculate the temperature t based on velocity of forward shock and other physical parameters.
...
def planck_fxn(filter, t):
    ...
    This function calculates the spectral radiance of a black body at a given temperature T for a specific frequency filter.
    
    
    Parameters
    ----------
    filter (array-like): The frequency values for which to calculate the spectral radiance.
    T (float): The temperature of the black body in Kelvin. Taken from the if functions depending on velocity of forward shock.
    Returns
    -------
    B : array-like
...
wien_r = wien_fxn(r_freq)
wien_g = wien_fxn(g_freq)
wien_i = wien_fxn(i_freq) 
wien_u = wien_fxn(u_freq)
wien_y = wien_fxn(y_freq)
wien_z = wien_fxn(z_freq)
